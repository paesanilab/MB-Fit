    ...
    with Database(database_name) as database:
        ...
        try:
            ...
            if len(all_opt_energies) > 1:
                raise MultipleOptimizedEnergiesError(database.file_name, molecule_name, method, basis, cp, tag, len(all_opt_energies))
...
def generate_2b_training_set(settings, database_name, output_path, monomer_1_name, monomer_2_name, method, basis, cp, tag):
    ...
    with Database(database_name) as database:
        ...
        try:
            ...
            if len(all_opt_energies) > 1:
                raise MultipleOptimizedEnergiesError(database.file_name, monomer_1_name, method, basis, cp, tag, len(all_opt_energies))
        ...
        try:
            ...
            if len(all_opt_energies) > 1:
                raise MultipleOptimizedEnergiesError(database.file_name, monomer_2_name, method, basis, cp, tag, len(all_opt_energies))
