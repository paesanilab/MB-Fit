...
def generate_training_set(settings, database_name, output_path, method, basis, cp):
    """
    ...
    molecule_energy_pairs = database.get_energies(method, basis, cp, "%")
    ...
    try:
        opt_energy = database.get_energies(method, basis, cp, "opt")[0][1][0]
    except IndexError:
