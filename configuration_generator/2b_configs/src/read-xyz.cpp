...
void read_frame(size_t& lineno, std::istream& is, tools::xyz_frame& frame,
		bool do_bind)
{
    assert(is && !is.eof());
	...
	iss >> tmpA;
    	if (iss.fail()) {
    		...
    		<< " of the XYZ file:" << std::endl << iss.str() << std::endl;
...
size_t read_xyz
    (const char* filename, std::vector<xyz_frame>& frames, bool do_bind)
{
    assert(filename);

    std::ifstream ifs(filename);
    if (!ifs)
        throw std::runtime_error("could not open XYZ file for reading");
...
size_t read_connectivity(const char* filename, std::vector<int *>& bonds,
                         std::vector<int *>& angles, std::vector<int *>& improper_dh,
                         std::vector<int *>& dihedrals, std::vector<int *>& dh_params) {
  assert(filename);
  std::ifstream ifs(filename);
  if (!ifs)
    throw std::runtime_error("could not open connectivity file for reading");
...
size_t read_intra_lj(const char* filename, std::vector<int*>& pairs, std::vector<double*>& lj) {
  ...
  std::ifstream ifs(filename);
  if (!ifs)
    throw std::runtime_error("could not open intramolecular file for reading");
...
size_t read_c6(const char* filename, std::vector<int*>& pairs, std::vector<double>& c6) {
  ...
  std::ifstream ifs(filename);
  if (!ifs)
    throw std::runtime_error("could not open c6 file for reading");
...
size_t read_molinfo(const char* filename, std::vector<double>& chg, 
                    std::vector<double>& pol, std::vector<int>& atomtype) {
  assert(filename);
  std::ifstream ifs(filename);
  if (!ifs)
    throw std::runtime_error("could not open connectivity file for reading");
...
size_t read_molinfo(const char* filename, std::vector<double>& chg,
                    std::vector<double>& pol, std::vector<double>& polfac,  std::vector<int>& atomtype) {
  assert(filename);
  std::ifstream ifs(filename);
  if (!ifs)
    throw std::runtime_error("could not open connectivity file for reading");
...
void print_xyz(std::ostream& os, std::vector<std::string> elements, 
	       const double* x, std::string comment)
{
	...
	os << std::setw(5) << std::left << name
   	   << std::setw(25) << std::right << x[n3 + 0]
   	   << std::setw(25) << std::right << x[n3 + 1]
   	   << std::setw(25) << std::right << x[n3 + 2]
   	   << std::endl;
    }
