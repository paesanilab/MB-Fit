...
namespace {
  ...
  const double max_dist = 10.0;//Angstrom
...
void get_confs(const double R,
  ...
  const double d1, std::ofstream &ofs) {
  ...
  for (size_t i = 0; i < ns; i++) { 
    ...
    std::copy(mon1[0].at_name.begin(), 
              mon1[0].at_name.end(), ats.begin());
    std::copy(mon2[0].at_name.begin(), 
              mon2[0].at_name.end(), ats.begin() + nat1);
...
int main(int argc, char** argv) {
  ...
  if (argc < 6) {
    std::cout << "Usage: " << argv[0] << std::endl
              << " <eq_geom1.xyz> <eq_geom2.xyz> " << std::endl
  ...
  for (double R = 0.6; R < max_dist; R += dR) {
    get_confs(R, set1, set2, ns, prg, d1, ofs);
