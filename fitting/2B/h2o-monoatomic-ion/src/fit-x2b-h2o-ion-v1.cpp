...
typedef h2o_ion::x2b_h2o_ion_v1<5> model_type;
...
#endif
#ifdef FLUORIDE
//const double E_range = 90.0; // kcal/mol
const double E_range = 50.0; // kcal/mol
#endif
#ifdef CHLORIDE
const double E_range = 40.0; // kcal/mol
#endif
#ifdef BROMIDE 
const double E_range = 40.0; // kcal/mol
#endif
...
static std::vector<h2o_ion::dimer> training_set;
...
int main(int argc, char** argv)
{
    ...
    if (argc < 3) {
        std::cerr << "usage: fit-x2b-h2o-ion-v1 kvs ts1 ..."
                  << std::endl;
        return 0;
    }
...
#   endif
    ...
    {
        const char fn[] = "fit-x2b-h2o-ion-v1-initial.cdl";
        std::ofstream ofs(fn);
    ...
    try {
        while (--argc != 0) {
            size_t nd = h2o_ion::load_dimers(*argv, training_set);
            std::cout << "'" << *(argv++) << "' : "
                      << nd << " dimers" << std::endl;
        }
    ...
    h2o_ion::setup_weights(training_set, E_range, E_min,
    ...
    {
        const char fn[] = "fit-x2b-h2o-ion-v1.cdl";
        std::ofstream ofs(fn);
        std::cout << "\n>> saving as '" << fn << "'\n";
        model.as_cdl(ofs);
    }
