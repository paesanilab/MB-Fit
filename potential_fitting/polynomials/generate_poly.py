...
def parse_input(input_path):
    ...
    Args:
        input_path - path to the poly.in input file

    Returns:
        fragments and variables as read from the input file
    """
...
def make_permutations(fragment):
    ...
    Args:
        fragment in he form "A1B2", "A3", etc.
...
def combine_permutations(fragments, fragment_permutations):
    ...
    Works by generating one permutation for every possible choice of 1 permutation from each fragment, but if 2 fragments are the same,
    their permutations will also be swapped in with eachother to create all permutations where equivelent atoms are swapped in every possible order.

    Args:
        fragments - a list of strings of format "A1B2" representing the fragments
    ...
    for i in range(1, len(fragments)):
        ...
        if fragments[i] == fragments[0]:
            ...
            for permutation in fragment_permutations[i]:
                ...
                # the reason we swap the permutations is because identical fragments DO NOT have the same numbers in their permutations as the index numbers are indices in the molecule, so the 2nd fragment will have
...
def make_variable_permutations(variables, atom_permutations, atom_names):
    ...
    Args:
        varibles - a list of all variable objects
        atom_permutations - a list of all the atom permutations as generated by combine_permutations()
        atom_names - a list of the names of each atom in this molecule, unique for ech atom

    Returns:
        array of indicies representing the variables in this fragment
    """
...
def generate_monomials(number_of_vars, degree):
    ...
    Args:
        number_of_vars - the number of varibles to generate monomials for
        degree - the degree of each monomial

    Returns:
        A list of all possible lists of length number of vars with entries adding to degree
    """
...
def permute_monomial(monomial1, variable_permutations):
    ...
    Args:
        monomial1 - the monomial to permute
        variable_permutations - the permutations of the variables in the monomial as geneated by make_variable_permutations

    Returns:
        List of all monomials created by permuting monomial1 by all the variable_permutations
    """
