...
def generate_input_poly(settings_file, poly_in_path):
    ...
    with open(files.init_file(poly_in_path), "w") as poly_in_file:
        ...
        # loop thru each type list and generate each fragment's set of uniquely
        # named atoms
        sets_list = []
        ...
        for fragment in fragments:
            ...
            # loop thru every other character in fragment, get the letters
            # (A, B, C) but not numbers (1, 2, 3)
            for i in range(0, len(fragment), 2):

                # check if there is only one of this atom in this fragment by
                # looking at the character after the letter, which is a number
                # signifying how many of that atom are in this fragment
                if (int(fragment[i + 1]) == 1):

                    # check if this atom is not a virtual site
                    if not fragment[i] in virtual_sites:

                        # add it to the fragment's set without a number in
                        # format A__a
                        frag_set.append(fragment[i] + '_' +  '_' + chr(letter))

                # otherwise there is more than one of this atom in this
                # fragment
                else:

                    # counter to count how many of this atom are in the
                    # fragment
                    num_atoms = 1

                    # loop thru as many atoms as there are of this type in this
                    # fragment
                    for j in range(int(fragment[i + 1])):
                        ...
                        if not fragment[i] in virtual_sites:
                            ...
                            # add it to the fragment's set with a number in
                            # format A_1_a
                            frag_set.append(fragment[i] + '_' + str(num_atoms)
                                    + '_' + chr(letter))
                            num_atoms += 1

            # loop thru every character in fragment again, this time to look
            # for virtual sites
            for i in range(0, len(fragment), 2):

                # counter to count how many of this virtual site are in the
                # fragment
                num_atoms = 1
                ...
                if fragment[i] in virtual_sites:
                    ...
                    # look one character ahead to see how many of these virtual
                    # sites are in the fragment
                    for j in range(int(fragment[i + 1])):
            ...
            # increment the letter, so next fragment has next lowercase letter
            # alphabetically
            letter += 1
        ...
        for frag_set in sets_list:
            ...
            for i in range(0, len(frag_set) - 1):
                for j in range(i + 1, len(frag_set)):
                    ...
                    # split the first atom along _ to seperate it: "A_1_a" ->
                    # ["A", "1", "a"] or "A__a" -> ["A", "", "a"]
                    split_atom1 = frag_set[i].split('_')
                    # split the second atom along _ to seperate it: "A_1_a" ->
                    # ["A", "1", "a"] or "A__a" -> ["A", "", "a"]
                    split_atom2 = frag_set[j].split('_')

                    # combine first element of each split atom in alphabetical
                    # order to get something like "AA", or "AB"
                    ...
                    if not split_atom1[0] in virtual_sites and not \
                        ...
                        poly_in_file.write("add_variable['" + split_atom1[0]
                                + split_atom1[1] + "', '" + split_atom1[2]
                                + "', '" + split_atom2[0] + split_atom2[1]
                                + "', '" + split_atom2[2] + "', 'x-intra-"
                                + combined_letters + "']\n")
        
        # loop thru every pair of fragments and add write variables to file for
            ...
            for frag_set2 in sets_list[sets_list.index(frag_set1) + 1:]:
                ...
                for i in range(0,len(frag_set1)):
                    for j in range(0,len(frag_set2)):
                        ...
                        # combine first element of each split atom in
                        # alphabetical order to get something like "AA", or
                        # "AB"
                        combined_letters = ''.join(sorted(split_atom1[0]
                                + split_atom2[0]))
                        poly_in_file.write("add_variable['" + split_atom1[0]
        ...
        polynomial_filtering = settings.get("poly_generation",
                "accepted_terms",
                "all" if len(fragments) == 1 else "purely-inter")

        # make sure the user hasn't chosed intermolecular terms only with a
        # monomer
        if len(fragments) == 1 and not polynomial_filtering == "all":
            raise InconsistentValueError("number of fragments",
                    "[poly_generation].accepted_terms", len(fragments),
                    polynomial_filtering, 
                    "when there is only 1 fragment, there are no intermolecular interactions, so you must use 'all' terms")

        if polynomial_filtering == "purely-inter":
            # this filter filters out all terms that have any intra-molecular
            # components
            poly_in_file.write("add_filter['degree', 'x-intra-**', '1+', '*']")
        elif polynomial_filtering == "partly-inter":
            # this filter filters out all terms that have no inter-molecular
            #components
            poly_in_file.write(
                    "add_filter['not', 'degree', 'x-**', '1+', '*']")

        elif polynomial_filtering != "all":
            raise InvalidValueError("[poly_generation][accepted_terms]",
                    polynomial_filtering,
